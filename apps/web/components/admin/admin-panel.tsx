...
function PaymentsSection({ payments, onPageChange, working }: { payments: Paginated<Payment>; onPageChange: (page: number) => Promise<void>; working: boolean }) {
  ...
  return <section className="admin-panel"><div className="admin-panel-head"><div><h2>پرداخت‌های آنلاین</h2><p>گزارش وضعیت تراکنش‌های ثبت‌شده توسط درگاه پرداخت</p></div></div><div className="admin-table-wrap"><table className="admin-table"><thead><tr><th className="admin-row-number">ردیف</th><th>شناسه درخواست</th><th>روش</th><th>مبلغ</th><th>مرجع تراکنش</th><th>تاریخ</th><th>وضعیت</th></tr></thead><tbody>{payments.items.map((item, index) => <tr key={item.id}><td className="admin-row-number">{(startIndex + index + 1).toLocaleString('fa-IR')}</td><td dir="ltr">{item.nazrRequestId.slice(0, 8)}</td><td>آنلاین</td><td>{money(item.amount)}</td><td dir="ltr">{item.transactionReference ?? '—'}</td><td>{date(item.createdAt)}</td><td><span className={`admin-status ${statusClass(item.status)}`}>{paymentLabels[item.status]}</span></td></tr>)}</tbody></table>{!payments.items.length ? <Empty /> : null}</div><AdminPagination info={payments} onPageChange={onPageChange} working={working} /></section>;
}
