...
function GallerySection({ items, nazrTypes, run, working }: { items: GalleryAsset[]; nazrTypes: NazrType[]; run: Runner; working: boolean }) {
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const form = event.currentTarget; const data = new FormData(form); if (await run(() => createAdminGallery({ title: String(data.get('title')), type: String(data.get('type')) as 'image' | 'video', fileUrl: String(data.get('fileUrl')), thumbnailUrl: String(data.get('thumbnailUrl')) || null, nazrTypeId: String(data.get('nazrTypeId')) || null }), 'رسانه به گالری اضافه شد')) form.reset(); };
